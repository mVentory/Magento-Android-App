...
public class LoadOrderDetailsData extends BaseTask<OrderDetailsActivity, Map<String, Object>>
        implements
    ...
    protected Integer doInBackground(Object... args) {
        if (mRefresh || JobCacheManager.orderDetailsExist(new String[] {
            mOrderIncrementId
        }, mSettingsSnapshot.getUrl()) == false) {
            mResHelper.registerLoadOperationObserver(this);
            mOrderDetailsReqId = mResHelper.loadResource(getHost(), RES_ORDER_DETAILS,
                    new String[] {
                        mOrderIncrementId
                    }, mSettingsSnapshot);
            mNLatches += 1;
        ...
        if (mSuccess) {
            myData = JobCacheManager.restoreOrderDetails(new String[] {
                mOrderIncrementId
