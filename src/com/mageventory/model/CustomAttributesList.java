...
public class CustomAttributesList implements Serializable, MageventoryConstants {
	...
	public CustomAttributesList(Activity activity, ViewGroup parentViewGroup,
			...
			for (CustomAttribute elem : listCopy) {
				if (elem.getAttributeID().equals(customAttr.getAttributeID())
					...
					if (customAttr.isOfType(CustomAttribute.TYPE_BOOLEAN)
							...
							|| customAttr
									.isOfType(CustomAttribute.TYPE_MULTISELECT)) {
						// restore options
						for (CustomAttributeOption option : elem.getOptions()) {
							if (option.getSelected() == true) {
								for (CustomAttributeOption optionNew : customAttr
										.getOptions()) {
									if (optionNew.getID()
											.equals(option.getID())) {
						...
						customAttr.setSelectedValue(elem.getSelectedValue(),
								false);
					}
	...
	private String removeCodeFromCompoundName(String compoundName, String code) {
		...
		if (indexBegin != -1) {
			...
			if (indexBegin > 0) {
				if (compoundName.charAt(indexBegin - 1) == ' '
						|| compoundName.charAt(indexBegin - 1) == '(') {
					indexBegin--;
				}
			}

			if (indexEnd < compoundName.length()) {
				if (compoundName.charAt(indexEnd) == ','
						|| compoundName.charAt(indexEnd) == ')') {
					indexEnd++;
				}
			}
			compoundName = compoundName.replace(
					compoundName.substring(indexBegin, indexEnd), "");
		}
	...
	private static class CreateOptionTask extends
		...
		protected Integer doInBackground(Void... params) {
			...
			requestId = resHelper.loadResource(host,
					RES_PRODUCT_ATTRIBUTE_ADD_NEW_OPTION, new String[] {
	...
	public static interface OnNewOptionTaskEventListener {
		...
		void OnAttributeCreationFinished(String attributeName,
	...
	public void showAddNewOptionDialog(final CustomAttribute customAttribute) {
		...
		alert.setMessage("Enter a name for a new option for \""
				+ customAttribute.getMainLabel() + "\" attribute.");
		alert.setView(textEntryView);

		final EditText editText = (EditText) textEntryView
				.findViewById(R.id.newOptionEditText);
	...
	private void showDatepickerDialog(final CustomAttribute customAttribute) {
		...
		try {
			...
			final Date d = f.parse(((EditText) customAttribute
					.getCorrespondingView()).getText().toString());
			c.setTime(d);
		...
		final Dialog d = new DatePickerDialog(mActivity, onDateSetL, year,
				month, day);
		d.show();
	}
	...
	private void showMultiselectDialog(final CustomAttribute customAttribute) {
		...
		final boolean[] checkedItems = new boolean[customAttribute.getOptions()
				.size()];
	...
	private View newAtrEditView(final CustomAttribute customAttribute) {
		...
		if (customAttribute.isOfType(CustomAttribute.TYPE_BOOLEAN)
			...
			if (customAttribute.getOptions() == null
					|| customAttribute.getOptions().isEmpty()) {
				return null;
			}
		...
		if (customAttribute.isOfType(CustomAttribute.TYPE_PRICE)
			...
			edit.addTextChangedListener(new TextWatcher() {
				...
				public void afterTextChanged(Editable s) {
					customAttribute.setSelectedValue(edit.getText().toString(),
							false);
					setNameHint();
				}
			});
