...
public class DialogUtil implements MageventoryConstants {
	...
	public static Dialog createCategoriesDialog(final Activity context,
			final Map<String, Object> rootCategory,
			final OnCategorySelectListener onCategorySelectL,
			final Category preselect) {
		...
		try {
			...
			final TreeViewList list = (TreeViewList) ((LayoutInflater) context
					...
					.inflate(R.layout.dialog_category_list, null);
			...
			final TreeBuilder<Category> treeBuilder = new TreeBuilder<Category>(
					treeStateManager);

			Util.buildCategoryTree(rootCategory, treeBuilder);
			final CategoryTreeAdapterSingleChoice adapter = new CategoryTreeAdapterSingleChoice(
					context, treeStateManager, 4);
			...
			if (preselect != null) {
				final int selection = context.getSharedPreferences(
						PREFERENCES_NAME, Context.MODE_PRIVATE).getInt(
						PKEY_SELECTION, 0);
	...
	public static Dialog createListDialog(final Activity host,
			final String dialogTitle, final List<Map<String, Object>> data,
			final int rowId, final String[] keys, final int[] viewIds,
			final OnItemClickListener onItemClickL) {
		final Dialog dialog = new Dialog(host);
		dialog.setTitle(dialogTitle);

		final ListView list = new ListView(host);
		final SimpleAdapter adapter = new SimpleAdapter(host, data, rowId,
				keys, viewIds);
