...
public class CreateShipmentProcessor implements IProcessor, MageventoryConstants {
	...
	public void process(Context context, Job job) {
		...
		Map<String, Object> params = (Map<String, Object>)requestData.get(EKEY_SHIPMENT_WITH_TRACKING_PARAMS);
		
		final Map<String, Object> orderDetails = client.shipmentCreate(orderIncrementId, carrierCode, title, trackingNumber, params);

		if (orderDetails != null) {
			JobCacheManager.storeOrderDetails(orderDetails, new String [] {orderIncrementId}, job.getJobID().getUrl());
		} else {
