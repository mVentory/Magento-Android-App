...
public class ResourceStateDao {
	...
	public boolean deleteResource(final String resourceUri) {
		return resolver.delete(CONTENT_URI, RESOURCE_URI + "=?",
				new String[] { resourceUri }) != 0;
	}

	public ResourceRepr getResource(final String resourceUri) {
		final Cursor cursor = resolver.query(CONTENT_URI, projection,
				RESOURCE_URI + "=?", new String[] { resourceUri }, null);
		if (cursor != null) {
			try {
				if (cursor.moveToFirst()) {
					...
					long timestamp;
					try {
						final String timestampStr = cursor
								.getString(timestampIdx);
					...
					return new ResourceRepr(state, timestamp, transaction,
							resourceUri, old);
				}
			} finally {
				cursor.close();
	...
	public boolean setState(final String resourceUri, int state) {
		...
		return resolver.update(CONTENT_URI, values, RESOURCE_URI + "=?",
				new String[] { resourceUri }) != 0;
	}
	 ...
	 */
	public static String buildParameterizedUri(final int resourceType,
			final String[] params) {
		// y: the exclamation mark is very important, it prevents LIKE queries
		// to match other resources with similar id
	...
	public boolean setOld(final int resourceType, String[] params,
			final boolean old) {
		final ContentValues values = new ContentValues(1);
		values.put(OLD, old ? 1 : 0);
