...
/**
 * Implementation of view splitter which allows to manually resize its children
 * via the handler. Implementation is taken from
 * https://github.com/k9mail/splitview and has some modification which should be
 * pulled back TODO
 */
public class SplitView extends LinearLayout implements OnTouchListener {
    static final String TAG = SplitView.class.getSimpleName();
    ...
    public int getPrimaryContentSize() {
        int result;
            if (getOrientation() == VERTICAL) {
            result = mPrimaryContent.getMeasuredHeight();
            } else {
            result = mPrimaryContent.getMeasuredWidth();
            }
        CommonUtils.debug(TAG, "getPrimaryContentSize: %1$d", result);
        return result;
    }
