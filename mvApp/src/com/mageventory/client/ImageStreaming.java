...
public class ImageStreaming {
    ...
    /**
     * Create String contains XML Request for Uploading Image
     * "Part1 --> till base64"
     * 
     * @param method
     * @param sessionID
     ...
     * @return
     */
    private static String xmlRequestBuilderPartOne(String method, String sessionID, String apiName,
            Object[] data) {
    ...
    public static Object streamUpload(URL url, String method, String sessionID, String apiName,
        ...
        try {
            ...
            if (readBytes < imgFileSize) {
                ...
                base46_length += (new String(Base64Coder_magento.encode((byte[]) (Base64.encode(
                        remainingBytes,
            ...
            /*
             * String encodedDataStr = new
             * String(Base64Coder_magento.encode((byte[])(Base64.encode(content,
             * Base64.DEFAULT)))); uploadStream.write(encodedDataStr);
             * uploadStream.flush();
             */
            readBytes = 0;
            ...
            for (int i = 0; i < counter; i++) {
                ...
                f.seek(readBytes);
                f.read(content);
                uploadStream.write(new String(Base64Coder_magento.encode((byte[]) (Base64.encode(
                        content,
                        Base64.DEFAULT)))));
                uploadStream.flush();
                readBytes += chunk_size;
    ...
    private static Object deserialize(XmlPullParser parser) throws XmlPullParserException,
            IOException {
